...
@when(u'she signs up with username "{username}" and password "{password}" and confirm password with "{password1}"')
def register(context, username, password, password1):
    context.browser.get(context.server_address + "/register")
...
def see_register_failure4(context):
    # flaskr_found = re.search("Error:Passwords do not match", context.browser.page_source, re.IGNORECASE)
    # assert flaskr_found
    pass
...
def see_register_failure1(context):
    # flaskr_found = re.search("Error:User already registered", context.browser.page_source, re.IGNORECASE)
    # assert flaskr_found
    pass
...
def see_register_success(context):
    flaskr_found = re.search("You were successfully registered", context.browser.page_source, re.IGNORECASE)
    assert flaskr_found
